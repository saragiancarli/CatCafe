...
import view.ReviewAltView;

import java.util.List;

public class ReviewAltController {

    private final NavigationService nav;
    private final String            typeOfLogin;

    private final ReviewAltView  view;
    private final ReviewService  service = new ReviewService();
    private final Client         currentUser;

    public ReviewAltController(NavigationService nav, String typeOfLogin) {
        this.nav         = nav;
    ...
    private void handleSend() {
        ...
        if (!bean.hasValidStars())    { err("Le stelle devono essere un numero tra 1 e 5"); return; }
